...
to setup-regular-lattice
  clear-all
  resize-world 0 (initial-ws - 1) 0 (initial-ws - 1)

  ; initialize global variables
  set world-size initial-ws ; this prevents errors when the user changes world-size during simulation
  set init-value initial-wage * world-size * world-size
  setup
  ...
  ask patches [
    sprout 1 [
      set turtle-id who
      set color white
      ; initialize variables for each patch
      setup-generate-strategy    ; set the strategy of the employee
      setup-generate-diligence   ; set the diligence of the employee
      setup-set-strategy-label   ; sets the label for each employee based on their strategy
      setup-set-basic-values-for-employee ; sets all basic values for each employee
      set label-color red
    ]
  ]

  ; create link with all neighbours
  ask turtles [
    create-links-with turtles-on neighbors
  ]

  setup-count-all-strategies
  setup-history-lists     ; sets all lists needed for remembering the history of cooperation between employees
...
to go
  ...
  if (ticks mod boss-reaction-time) = 0 and ticks != 0 and retirement [
    ask turtles [
...
; Function sets the strategy of the employee. It is based on intial-cooperation slider that specifies
; the portion of employees with defect strategy. Then the rest is equally divided between
; cooperative strategies - cooperative, tit-for-tat and unforgiving
to setup-generate-strategy
  ifelse random-float 1.0 < initial-cooperation
      [ setup-cooperation ]
      [ set strategy "defect" ]
  end

; Function set the cooperative strategy of the employee
to setup-cooperation
  let rand-coop random-float 1.0
  ifelse rand-coop < 0.1666
    [ set strategy "cooperate" ]
    [ ifelse rand-coop < 0.3333
      [ set strategy "tit-for-tat-npm" ]
      [ ifelse rand-coop < 0.5
        [ set strategy "tit-for-tat" ]
        [ ifelse rand-coop < 0.6666
          [ set strategy "tit-for-two-tats" ]
          [ ifelse rand-coop < 0.8333
            [ set strategy "pavlov" ]
